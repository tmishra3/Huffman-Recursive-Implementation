...
HTREE* searchForestMin(HTREE* hf) 
{
	...
	{/* initialize minNode to first non-empty element of the forest*/
        ...
        //return searchForestMinHelp(hf, hf);
	}
}
...
int main(int argc, char *argv[]) {
	...
	char tempStr[20] = {0}; /*string for saving char:weight for a single character*/
  
	
    

	int i = 0; /* counter */
	int totalLength = 0; /* total length of contents of inputFile */
	...
	*/
	/*delete following lines when code is ready for compile on linux*/
	mode = "-d";
	filename = "test0.txt.hc";
		 ...
		 */
          ...
          */
           ...
           for (i = 0; i<strlen(inputText); i++)
		   {
		       ...
		       key = inputText[i];
		       //treelevel = 0; /*reset global variable for path recording - left or right*/
